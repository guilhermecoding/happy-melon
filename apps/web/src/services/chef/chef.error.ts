export class ChefServiceError extends Error {
  constructor(
    ...
    super(message);
    this.name = 'ChefServiceError';
  }
}

export async function parseChefError(
  response: Response,
  fallback: string,
): Promise<ChefServiceError> {
  const rawMessage = await readResponseMessage(response);
  return new ChefServiceError(
    response.status,
    mapChefErrorMessage(response.status, rawMessage, fallback),
  );
}

export function normalizeChefError(
  error: unknown,
  fallback: string,
): ChefServiceError {
  ...
  if (error instanceof Error && error.message.trim()) {
    return new ChefServiceError(null, error.message);
  }

  return new ChefServiceError(null, fallback);
}

export function getChefErrorMessage(error: unknown, fallback: string): string {
  return normalizeChefError(error, fallback).message;
}

function mapChefErrorMessage(
  status: number,
  rawMessage: string,
  fallback: string,
): string {
